...
export function ImageUpload({ onColorsExtracted }: ImageUploadProps) {
  ...
  const extractColors = (imgElement: HTMLImageElement) => {
    try {
      ...
      // Quality 1 = highest quality (samples more pixels for better accuracy)
      // Extract more colors initially to get better diversity
      const palette = colorThief.getPalette(imgElement, 8, 1);
        ...
        const hexColors = palette
          ...
          .filter((_color, index) => {
            ...
            for (let i = 0; i < index; i++) {
              const prevRgb = palette[i];
              const currRgb = palette[index];
              // Calculate simple color distance
              const distance = Math.sqrt(
                Math.pow(prevRgb[0] - currRgb[0], 2) +
                Math.pow(prevRgb[1] - currRgb[1], 2) +
                Math.pow(prevRgb[2] - currRgb[2], 2)
              );
              // If too similar (distance < 40), skip this color
              if (distance < 40) return false;
            }
            return true;
          })
          .slice(0, 5); // Take only the first 5 unique colors
        ...
        {!image ? (
          ...
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground text-center">
              Colors extracted successfully! Check the main palette in the "Palette" tab.
            </p>
            <Button
              variant="outline"
              size="sm"
